...
class Pigeon:
    ...
    def add_jobs(self):
        ...
        self.scheduler.add_job(self.check_update, trigger='cron', args=[0], hour=17, minute=30)
        self.scheduler.add_job(self.check_update, trigger='cron', args=[1], hour=17, minute=45)
        self.scheduler.add_job(self.check_update, trigger='cron', args=[2], hour=18, minute=00)
        self.scheduler.add_job(self.check_update, trigger='cron', args=[3], hour=18, minute=15)
    ...
    def check_update(self, time):
        ...
        for name, is_update in user_dict:
            if not is_update:
    ...
    def _send_alert(time, user):
        ...
        mail_msg += "没有写日报哦~ \n\n若一直没写日报，本条信息将从17:30至19:00每间隔10min，提醒一次~\n\n若至19:00还未写日报，将留空您今天的日报"
